  ...
  return (
    <div className="w-full sm:w-1/2 md:w-1/4 lg:w-25% bg-white rounded-2xl shadow-2xl m-4">
      <div className="flex p-4">
        <div>
          <h2 className="text-xl font-bold">{props.orderData && props.orderData.customer_name}</h2>
          <p className="text-sm font-bold">{props.orderData && props.orderData.mobile_number} | {props.orderData && props.orderData.customer_address}</p>
        </div>
      </div>
      ...
      <div className="flex justify-between px-4">
        <label className="justify-left">{props.orderData && limit(props.orderData.delivery_date,10)}</label>
        <label className="justify-right mr-8"> {props.orderData && props.orderData.delivery_time}</label>
      </div>
